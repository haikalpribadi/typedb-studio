...
open class SchemaThingType(
    ...
    private fun reloadEntries(transaction: TypeDBTransaction) {
        val new = concept.asRemote(transaction).subtypes.toList().toSet()
    ...
    private fun schemaThingTypeOf(concept: ThingType, transaction: TypeDBTransaction): SchemaThingType {
        val isExpandable = concept.asRemote(transaction).subtypes.findAny().isPresent
    ...
    class Root(concept: ThingType, session: SessionState) : SchemaThingType(concept, null, session, true),
